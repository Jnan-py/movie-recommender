...
def main():
    ...
    d = r"C:\Users\jnany\Downloads\movie_poster.csv"
    ...
    def poster(movie_id):
        try:
            ...
            img = plt.imread(r"C:\Users\jnany\Pictures\not_found.png")
        return img
    ...
    def fetch_poster(movie_id):
        try:
            ...
            ff = plt.imread(r"C:\Users\jnany\Pictures\not_found.png")
        return ff
    ...
    #     url = f"https://en.wikipedia.org/api/rest_v1/page/summary/{query}(film)"
    #     response = requests.get(url)
    #     if response.status_code == 200:
    #         data = response.json()
    ...
    #         return summary
    #     else:
    #         return "Could Not find the Summary."

    # def get_summary(movie_name):
    #     page = wp.page(movie_name)
    #     return page.summary 

    if opt == 'Search Movie':
        st.header('Search For Movie')
        data_mb1 = pd.read_csv(r"C:\Users\jnany\Downloads\10000 Movies Data\10000 Movies Data")
        ...
        if st.button('Search'):
            ...
            # qry = search_wikipedia(f_m)
            # summ = get_summary(qry)
            cols[0].image(img)
            cols[1].header(f_m)
            ...
            # cols[1].markdown(f"<a href={link} target='_blank' id='{link_id}' style='display: inline-block; padding: 10px 20px; background-color: #1D77BF; color: white; text-align: center; text-decoration: none; margin: 4px 2px; cursor: pointer; border-radius: 10px;'>More Info About {f_m}</a>", unsafe_allow_html=True)
        ...
        if st.button('Go'):
            ...
            path = r'C:\Users\jnany\archiv(1)'
            data_mb1 = pd.read_csv(r"C:\Users\jnany\Downloads\10000 Movies Data\10000 Movies Data")
            data_mb = data_mb1[['Movie_id','title','vote_average']]

        #     data_mb['ratio'] = data_mb['vote_average']/data_mb['vote_average'].max()
        #     f_d_con = data_mb[['title','ratio']]

        #     u_d = r'C:\Users\jnany\archive\ml-100k'
        #     cls = [
        #     'id','title','release_date','video_release_date','imdb_url','unknown','action','adventure','animation','children',
        ...
        #     d_ms = d_ms[['id','title']]
        #     d_rts = pd.read_csv(os.path.join(u_d,'u.data'),names=['user_id','id','rating','time_stamp'],sep='\t',encoding='latin-1')
        #     d_rts = d_rts.drop(columns=['time_stamp'])
        #     d_rts['ratio'] = d_rts['rating']/d_rts['rating'].max()
        #     rts = d_rts.groupby(['id'])['ratio'].mean().to_frame()
        #     rts['title'] = d_ms['title']
        #     rts = rts.reset_index()
            
        #     fd1 = f_d_con
        #     fd2 = rts[['title','ratio']]
        #     finn = pd.concat([fd1,fd2],ignore_index=True)
            ...
            for row in range(num):
                ...
                # else:
                #     title = search_wikipedia(lst_[k_])
                #     img = poster(d_ms[d_ms['title']==lst_[k_]]['id'].values[0])
                ...
                # cols[1].markdown(f"<a href={link} target='_blank' id='{link_id}' style='display: inline-block; padding: 10px 20px; background-color: #1D77BF; color: white; text-align: center; text-decoration: none; margin: 4px 2px; cursor: pointer; border-radius: 10px;'>More Info About {lst_[k_]}</a>", unsafe_allow_html=True)
                button_code = f"<a href='{link}' target='_blank' style='display: inline-block; padding: 10px 20px; background-color: #1D77BF; color: white; text-align: center; text-decoration: none; margin: 4px 2px; cursor: pointer; border-radius: 10px;'>{link_id}</a>"
                k_+=1
                cols[1].markdown(button_code, unsafe_allow_html=True)
        ...
        # st.subheader('**Based on "tmdb_5000" dataset**') 

        data_mb = pd.read_csv(r"C:\Users\jnany\Downloads\10000 Movies Data\10000 Movies Data")
        ...
        if st.button('Search'): 
            ...
            for i in range(rows):
                ...
                for j in range(4):
                    index = i * 4 + j
                    if index < len(ans):
                        cols[j].image(pos[k], use_column_width=True)
